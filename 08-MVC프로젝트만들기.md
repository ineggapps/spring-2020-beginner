...
1. Spring Legacy Project를 선택하고 다음과 같이 입력

![스프링 레거시 프로젝트 만들기](https://i.ibb.co/2tTHyyN/02.png)

2. 해당 프로젝트 마우스 우클릭 > Maven > Update Project 선택

![Project Maven Update](https://i.ibb.co/h2W0jbv/mvnupdate.png)

3. Update Maven Project 항목에서 OK버튼을 눌러 업데이트를 실시한다.

- 반드시 프로젝트에서 JDK버전을 1.8로 설정하기 전에 업데이트를 수행해야 한다.
- Maven Update를 수행하면 다시 1.5버전대로 JDK 옵션이 초기화됨

![Project Maven Update](https://i.ibb.co/9YGVcpg/mvnupdate2.png)

4. Java Compiler와 Java Build Path을 1.8버전으로 다음과 같이 설정한다.
...
5. Project Facet에서 Java를 1.8로 Dynamic Web Module을 3.1로 변경한다.

   ![좌측에서 Project Facet항목을 선택하여 우측을 다음과 같이 설정한 후 Apply버튼을 눌러 설정 변경](https://i.ibb.co/nrHGPZf/03.png)

6. pom.xml에서 다음의 property와 dependency 항목을 추가 후 저장

- jar파일이 저장된 로컬 폴더는 %userProfile%\.m2\repository이다. 오류가 발생하면 이 폴더를 비우고 다시 다운로드 받으면 된다.
